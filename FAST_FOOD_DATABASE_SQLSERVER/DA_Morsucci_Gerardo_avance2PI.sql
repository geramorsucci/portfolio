...
-- ACTUALIZAR LA DIRECCION DE UNA SUCURSAL ESPECIFICA
UPDATE Sucursales SET Direccion = '1234 New Adress ST' WHERE SucursalID = 1;
...
DELETE FROM DetallesOrden WHERE OrdenID = 10;
...
-- Desactivar las restricciones de la clave externa.
ALTER TABLE DetallesOrden
NOCHECK CONSTRAINT FK__DetallesO__Produ__6383C8BA;

DELETE FROM Productos WHERE CategoriaID = 2;

-- Volver a activar las restricciones de la clave externa.
ALTER TABLE DetallesOrden
CHECK CONSTRAINT FK__DetallesO__Produ__6383C8BA; -- no se quieren eliminar las transacciones porque en algun  momento ocurrieron
...
SELECT COUNT(OrdenID) AS 'Regristos unicos'
FROM Ordenes;
